        ...
        #    courier = Courier.find_by(user: user)
           ...
           if  user.present? && user.valid_password?(password)
                ...
                if customer.present? && courier.present?
                    ...
                    return render json: {success: true, customer_id: customer.id, user_id: user.id, login: user.email}, status: :ok
                elsif courier.present?
                    return render json: {success: true, user_id: user.id, courier_id: courier.id, login: user.email}, status: :ok
                else    
